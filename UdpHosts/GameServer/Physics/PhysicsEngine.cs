...
namespace GameServer.Physics ;
    ...
    public class PhysicsEngine
    {
        ...
        public void ProjectileRayCast(Vector3 origin, Vector3 direction, CharacterEntity source, uint trace, Ammo ammo)
        {
            var speed = 500f;
            var maxRange = 500f;
            ...
            if (hitHandler.T < maxRange)
            {
                ...
                Console.WriteLine(
                    $"HitHandler {hitHandler.HitCollidable.Mobility} T {hitHandler.T} HitCollidable {hitHandler
                        .HitCollidable} at {hitPosition}");
                ...
                if (hitEntity != null)
                {
                    _projectileSim.OnProjectileImpact(source, ammo, hitPosition, hitEntity);
                }
            }
            else
            {
        ...
        private void SendDebugProjectileSpawn(CharacterEntity source,
            uint traceId,
            Vector3 origin,
            Vector3 direction,
            float speed)
        ...
        private void SendDebugProjectilePoseHit(CharacterEntity source,
            uint traceId,
