...
namespace DemoRenderer.UI
{
    ...
    public class ImageBatcher
    {
        ...
        public unsafe void Flush(Int2 screenResolution, ImageRenderer renderer)
        {
            foreach (var batch in batches)
            {
                renderer.Render(batch.Key, screenResolution, new Span<ImageInstance>(batch.Value.Instances.Span.Memory, batch.Value.Instances.Count));
