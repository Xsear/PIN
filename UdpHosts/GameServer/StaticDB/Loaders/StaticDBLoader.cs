            ...
            foreach (var prop in properties)
            {
                try
                {
                    if (prop.Index != -1)
                    {
                        ...
                        {
                            // Our SDB parser leaves null characters in strings, which can break things for us if we try to send one of these strings in a network message, so let's make sure that can't happen.
